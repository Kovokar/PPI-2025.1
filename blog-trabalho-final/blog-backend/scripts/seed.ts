import sql from "../db"; // Importar 'sql' diretamente
...
async function seedDatabase() {
  try {
    ...
    // Limpar tabelas existentes (executar cada DROP TABLE separadamente)
    console.log("Deletando tabelas existentes (se houver)...");
    await sql`DROP TABLE IF EXISTS comments CASCADE`; // CASCADE para remover dependências
    await sql`DROP TABLE IF EXISTS posts CASCADE`;
    await sql`DROP TABLE IF EXISTS users CASCADE`;
    console.log("Tabelas deletadas.");

    // Recriar tabelas (executar cada CREATE TABLE separadamente)
    ...
    // Reiniciar sequências (executar cada ALTER SEQUENCE separadamente)
    ...
    // Inserir comentários para algumas postagens
    ...
    // É importante fechar a conexão quando o script de seed termina
    await sql.end();
  }
}
